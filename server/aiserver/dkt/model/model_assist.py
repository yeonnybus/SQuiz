...
def main():
    ...
    parser.add_argument('--dataset', type=str, help='Dataset file', required=True) # 데이터셋 파일
    parser.add_argument('--splitfile', type=str, help='Split file', required=True) # training set, test set..
    ...
    parser.add_argument('--timewindow', type=int, help='Number of timesteps to process in a batch.', # 각 배치에서 처리되는 time step 수
...
def load_dataset(dataset, split_file):
    seqs, num_skills = read_file(dataset) # seqs는 학습자의 행동 기록, num_skills는 카테고리 저장됨.
